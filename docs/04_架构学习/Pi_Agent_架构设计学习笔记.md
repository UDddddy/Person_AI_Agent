...
> 对象：[badlogic/pi-mono](https://github.com/badlogic/pi-mono)（Pi Agent）· TypeScript/Bun 实现
> 说明：本文基于对 `packages/agent`、`packages/ai`、`packages/coding-agent` 源码的精读整理，用于学习一个生产级 Agent 的完整设计。所有函数名、常量、阈值均来自真实源码。
...
//    - 部分结果通过 tool_execution_update 事件流式上报（长任务进度）
...
export function estimateTokens(message: AgentMessage): number {
  ...
  // 图像:                   按 ESTIMATED_IMAGE_CHARS = 4800 字符估算
}
```
...
2. **可以切在 assistant 消息上**——如果这条 assistant 带 tool_calls，它后面的 toolResult 会跟着保留（因为切割点在其之前）。
...
collectEntriesForBranchSummary(session, oldLeafId, targetId):
  ...
  //    注意：compaction/branch_summary 条目即使超预算也尽量保留（它们小而关键）
  // 4. 生成结构化摘要，前缀加一句
